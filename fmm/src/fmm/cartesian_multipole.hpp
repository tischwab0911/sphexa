...
void P2M_add(const T1* x, const T1* y, const T1* z, const T2* m, LocalIndex begin, LocalIndex end,
...
template<class T>
CartesianMultipole<T> P2M_finalize(CartesianMultipole<T> gv)
{
...
void addQuadrupole(CartesianMultipole<T>& composite, Vec3<Tc> dX, const CartesianMultipole<T>& addend)
{
    Tc rx = dX[0];
    Tc ry = dX[1];
    Tc rz = dX[2];
...
void M2M(int begin, int end, const Vec4<T>& Xout, const Vec4<T>* Xsrc, const CartesianMultipole<Tm>* Msrc,
 ...
 */
template<class T>
void M2L(const Vec3<T>& targetCenter, const Vec3<T>& sourceCenter, const CartesianMultipole<T>& multipole,
         CartesianLocalExpansion<T>& local)
{
...
Vec4<Ta> L2P(Vec4<Ta> acc, const Vec3<Tc>& target, const Vec3<Tc>& center, const CartesianLocalExpansion<Tm>& local)
{
