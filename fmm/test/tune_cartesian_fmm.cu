/*
 * Cartesian FMM parameter tuning benchmark for GPU dual traversal
 *
...
/*! @file
 * @brief Parameter tuning benchmark for GPU Cartesian quadrupole FMM dual traversal
 *
 * @author Timo Schwab <tischwab@ethz.ch>
 *
 * Sweeps over numWarps and TraversalConfig parameters using REAL Cartesian
 * quadrupole M2L and P2P workloads rather than the synthetic fmaf P2P loop
 * in tune.cu.  The Cartesian M2L is ~4x cheaper than the spherical harmonic
 * M2L (10 atomicAdd on reals vs ~42 atomicAddComplex), so the optimal
 * producer/consumer balance is likely different.
 *
...
using TuneWarpList = std::integer_sequence<int, 4>;
    ...
    unsigned nWarm, unsigned nRuns)
{
...

    {
        ...
        fmm::downsweepLocalExpansionsGpu(levelRange, d_childOffsets, d_centers, d_locals);
        ...
        if (numLeafRange > 0)
        {
            ...
            fmm::l2pKernel<<<(numLeafRange + numThreads - 1) / numThreads, numThreads>>>(
                ...
                d_ppot, d_pax, d_pay, d_paz, firstTarget);
            checkGpuErrors(cudaDeviceSynchronize());
        }
                           ...
                           unsigned numRuns      = 10)
{
    ...
    std::strftime(resultFileName, sizeof(resultFileName), "cart-fmm-tune-%Y%m%d-%H%M%S.txt", &tmNow);
    ...
    printf("════════════════════════════════════════════════════════════════════════════════\n");
    printf("  Cartesian Quadrupole FMM Dual Traversal Parameter Tuning Benchmark\n");
    printf("════════════════════════════════════════════════════════════════════════════════\n");
    ...
    cstone::setMacGpu(treeBuilder.nodeKeys(), numNodes, rawPtr(d_centers), invTheta, box);
    ...
    // Free BH-only device memory (no longer needed)
    ...
    thrust::device_vector<ryoanji::Vec3<T>> d_geoCenters(numNodes);
    thrust::device_vector<ryoanji::Vec3<T>> d_geoSizes(numNodes);
    ...
    thrust::device_vector<fmm::CartesianMultipole<T>> d_multipoles(numNodes);
    checkGpuErrors(cudaMemset(rawPtr(d_multipoles), 0, numNodes * sizeof(fmm::CartesianMultipole<T>)));

    fmm::computeLeafMultipolesGpu(rawPtr(d_x), rawPtr(d_y), rawPtr(d_z), rawPtr(d_m),
                                   treeBuilder.leafToInternal(), numLeaves, treeBuilder.layout(),
                                   rawPtr(d_centers), rawPtr(d_multipoles));
    fmm::upsweepMultipolesGpu(levelRangeSpan, treeBuilder.childOffsets(), rawPtr(d_centers), rawPtr(d_multipoles));
    checkGpuErrors(cudaDeviceSynchronize());

    // Allocate locals and particle accumulators (re-zeroed per config run)
    thrust::device_vector<fmm::CartesianLocalExpansion<float>> d_locals(numNodes);
    thrust::device_vector<float> d_ppot(numTargets), d_pax(numTargets), d_pay(numTargets), d_paz(numTargets);
    ...
    // Compute FMM p90/p99 vs BH (run first FMM config's full pipeline to get accelerations)
    double fmmP90 = 0, fmmP99 = 0;
    {
        // Run the GPU-native FMM overload to get accelerations for accuracy check
        ...
        fmm::computeGravityFMMGpu<fmm::DirectionalMac>(
            ...
            &fmmEnergy, numParticles, nullptr);
    ...
    printDual(resultFile, "════════════════════════════════════════════════════════════════════════════════"
                          "════════════════════════════════════════════════════════════════════════════════\n");
    printDual(resultFile, "  Cartesian Quadrupole FMM Dual Traversal Parameter Tuning Results\n");
