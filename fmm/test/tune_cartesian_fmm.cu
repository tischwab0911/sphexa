                           ...
                           unsigned numRuns      = 10)
{
    ...
    // Compute Cartesian multipoles (P2M + M2M) on CPU via ADL
    ...
    printf("\nRunning CPU Cartesian FMM reference...\n");
...
#pragma omp parallel for schedule(dynamic)
    ...
    fmm::CartesianMultipole<T>* d_multipoles;
    checkGpuErrors(cudaMalloc(&d_multipoles, numNodes * sizeof(fmm::CartesianMultipole<T>)));
    checkGpuErrors(cudaMemset(d_multipoles, 0, numNodes * sizeof(fmm::CartesianMultipole<T>)));
