...
/*! @file
 * @brief Test Cartesian quadrupole FMM (dual traversal) against direct sum
 *
 * Builds an octree, computes Cartesian quadrupole multipoles,
 * runs a dual-traversal FMM (M2L + P2P + L2L + L2P), and compares
...
TEST(CartesianFMM, VsDirectSum)
{
    ...
    // compute Cartesian quadrupole multipoles (P2M + M2M upsweep via ADL)
    ...
    // FMM tree walk (dual traversal + L2L downsweep + L2P)
    ...
    std::cout << "Cartesian FMM (quadrupole) for " << numParticles << " particles: " << elapsed << " s" << std::endl;
